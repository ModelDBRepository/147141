...
def info(filepath, cells, noinhib):
    ...
    #numtrials = int((auxvars.GetInfoDur / stimlength) / 2) # Number of 'unique' and 'repeat' trials
    ...
    if numsegs() * buffertime < auxvars.infotriallength:
        print "Not enough spike segments to obtain pre-deletion information (needs at least %d s)" % (auxvars.deletionstart/1000)
    else:   
        uniquesfile = open("%s/info-uniques.csv" % filepath, 'w')
        repeatsfile = open("%s/info-repeats.csv" % filepath, 'w')
        
        for i in range(int(auxvars.GetInfoDur / buffertime)):
            print "Scanning segment %d to add to CSV file" % i
            ...
            for index in range(len(data[0])):
                ...
                if cellID in cells:
                    trialnum = int(spiketime / (stimlength * 2)) # Obtain trial number.
...
# deletionscale
# Plots scaling factors of cells at time-of-deletion, vs total time
def deletionscale(filepath):
    global titleson # Declare that we want to use the global 'titleson' variable
    cells = range(auxvars.numcells)
    ...
    for cellnum in range(len(cells)):
        ...
        for entrynum in range(len(celldata)):
            if int(celldata[entrynum][DEAD]) or entrynum >= len(celldata)-1:
                ...
                xaxis.append(auxvars.t_start + (entrynum * auxvars.recording_interval)) # Add time to x-axis
    ...
    # Draw labels
    pyplot.xlabel("Time (ms)")
    pyplot.ylabel("Scale factor at time of deletion")
    if titleson:
        pyplot.suptitle(filepath)
...
# Set index numbers for the aux file data lines (format: CELL,TYPE,ACTIVITY,TARGET,SCALE,DEAD)
global CELLID
CELLID = 0
global TYPE
TYPE = 1
